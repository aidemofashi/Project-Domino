...
while True:
    ...
    if res and res[0]['text'].strip():
        ...
        # 2. 读取并累加
        filename = "audio_input_file.json"
        chat_file = "chat.json"

        if os.path.exists(filename) and os.path.getsize(filename) and os.path.exists(chat_file) and os.path.getsize(filename)> 0:
        ...
        if Filter.emo(res[0]['text']) != False:
            ...
            chat.append('"role": "Aidemofashi", "content": '+ res[0] )
