...
# --- 计时辅助函数 ---
def log_duration(step_name, start_time):
    """打印步骤耗时"""
    duration = time.time() - start_time
    print(f"[TIMING] {step_name}: {duration:.3f} 秒")
...
def main():
    ...
    messege, _ = load_history()   # messege 用于发送给 LLM（含图片）
    # 启动后台 Vosk 线程
    ...
    while True:
        try:
            ...
            try:
                ...
                log_duration("ASR 识别", asr_start)

                date_time = time.strftime("%Y-%m-%d %H:%M:%S")
                print(f"[{date_time}] 用户: {user_text}")

                if user_text and isinstance(user_text, list) and len(user_text) > 0:
                    filtertext = user_text[0].get('text', '')
                    if not filtertext:
                        continue

                    filter_start = time.time()
                    if filter.emo(filtertext):
                        log_duration("过滤", filter_start)

                        # 截屏
                        screen_start = time.time()
                        image_data = shot_screen()
                        log_duration("截屏", screen_start)
                        ...
                        log_duration("LLM 响应", llm_start)

                        if response:
                            print("说话中...")
                            tts_start = time.time()
                            audio_output.text_to_speech(response)
                            log_duration("TTS 合成播放", tts_start)
                            ...
                            save_history(chat)
                            save_shot(shot)
                            log_duration("保存历史", save_start)

                            timer.mark_activity()

                        log_duration("用户请求总耗时", total_start)
                    else:
                        print(">>> 消息被 Filter 过滤。")
                else:
                    continue
                ...
                if timer.should_trigger(SILENCE_TIMEOUT):
                    ...
                    # 重新加载历史（确保数据最新）
                    chat, shot = load_history()
                    messege, _ = load_history()   # 重新构造发送消息列表
                    ...
                    screen_start = time.time()
                    image_data = shot_screen()
                    log_duration("截屏 (主动)", screen_start)
                    ...
                    llm_start = time.time()
                    response = llm_input.send_llm(messege)
                    log_duration("LLM 响应 (主动)", llm_start)
                    ...
                    if response:
                        ...
                        tts_start = time.time()
                        audio_output.text_to_speech(response)
                        log_duration("TTS 合成播放 (主动)", tts_start)
                        ...
                        save_history(chat)
                        save_shot(shot)   # ✅ 保存列表 shot，而非单个图片
                        log_duration("保存历史 (主动)", save_start)

                    # 标记已触发
                    timer.mark_trigger()
                    log_duration("主动触发总耗时", total_start)
